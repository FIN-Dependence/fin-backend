...
Chroma DB 생성 및 Colab 실행 방법은 `rag/README.md`와 `rag/COLAB_실행순서.md`에 정리되어 있습니다. 생성되는 `rag/chroma_db`는 저장소에 커밋하지 않습니다. 정확히 같은 DB 파일이 필요한 팀원에게는 `package_chroma.py`로 ZIP을 만들고 GitHub Release로 배포합니다.

Qwen3-Embedding-0.6B는 JSONL 근거를 벡터로 만드는 임베딩 모델이고 Chroma는 그 벡터를 저장·검색하는 DB입니다. 질문이 들어오면 Chroma가 관련 근거를 찾고, Java 백엔드가 설문과 근거를 합쳐 Gemini에 전달하는 구조입니다. 현재 Java 코드는 JSONL 키워드 검색 MVP이므로 Chroma/LangChain 검색 연결은 담당 팀원이 통합하는 다음 단계입니다.
