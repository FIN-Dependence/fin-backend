...
현재 RAG는 합쳐진 JSONL 파일을 키워드 검색하는 MVP 구현입니다. `RAG_ENABLED=true`와 `RAG_DATA_PATH`를 지정하면 근거 문장을 프롬프트에 넣고 출처를 응답합니다. 다음 단계에서는 `RagContextService`가 팀의 Chroma 검색 API를 호출하도록 교체합니다.
